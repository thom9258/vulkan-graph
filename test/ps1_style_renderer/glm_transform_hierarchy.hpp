...
static auto transform_child_from_parent_unity_style_2(glm::mat4 parent_global,
    ...
    glm::vec3 p_scale = glm::vec3(
        glm::length(glm::vec3(parent_global[0])), glm::length(glm::vec3(parent_global[1])),
        glm::length(glm::vec3(parent_global[2]))
    );
    ...
    glm::vec3 c_translation = glm::vec3(child_local[3]);
    glm::vec3 c_scale = glm::vec3(
        glm::length(glm::vec3(child_local[0])),
        glm::length(glm::vec3(child_local[1])),
        glm::length(glm::vec3(child_local[2]))
    );
    ...
    // Translate by the child's local offset (Unity children rotate with parent but don't shear from scale)
    global_matrix = glm::translate(global_matrix, c_translation);
    // Apply child local rotation
    global_matrix = global_matrix * c_rotation;
    // Apply the final combined scale cleanly to the basis axes using GLM's factory function
...
static auto inverse_location_unity_style_2(glm::mat4 location) -> glm::mat4 {
    ...
    // Build standard orthogonal inverse rotation matrix (transposing the orientation)
    glm::mat4 inv_rotation = glm::mat4(1.0f);
    ...
    glm::vec3 inv_translation = glm::vec3(inv_rotation * glm::vec4(-translation, 1.0f));
    inv_rotation[3] = glm::vec4(inv_translation, 1.0f);
...
static auto transform_child_from_parent_unity_style_3(glm::mat4 parent_global,
    ...
    if (p_scaleX > 0.0f) p_unscaled[0] /= p_scaleX;
    if (p_scaleY > 0.0f) p_unscaled[1] /= p_scaleY;
    if (p_scaleZ > 0.0f) p_unscaled[2] /= p_scaleZ;

    // 4. Strip scale from child to get pure local orientation/translation
    glm::mat4 c_unscaled = child_local;
    if (c_scaleX > 0.0f) c_unscaled[0] /= c_scaleX;
    if (c_scaleY > 0.0f) c_unscaled[1] /= c_scaleY;
    if (c_scaleZ > 0.0f) c_unscaled[2] /= c_scaleZ;
    ...
    glm::vec3 final_scale = glm::vec3(p_scaleX * c_scaleX, p_scaleY * c_scaleY, p_scaleZ * c_scaleZ);
    
    // Using the official GLM function ensures the scale parameters are properly set
